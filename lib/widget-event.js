...
class WidgetEvent extends EventEmitter {
	...
	addListener(selector, listener, capture) {
		...
		if (event.event) {
			/*
				TODO: Split DOM add listener code in to it's own method.
				This is needed so that `once`, `prependListener` and `prependOnceListener`
				can be implemented without duplicate code.
			 */
		...
		if (event.event) {
			...
			if (registered) {
				if (typeof listener !== 'function') {
					...
					/*
						TODO: Make sure that the listener is in the EventEmitter, if it's not
						and the DOM event counter is decreased, then the DOM events and Node
						events will get out of sync and weird things start to happen.
					 */
	...
	removeAllListeners(selector) {
		// TODO: Remove all matching DOM event listeners when removing all listeners
		throw new Error('NOT IMPLEMENTED');
	}
